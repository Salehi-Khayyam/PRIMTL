...
import jdd.JDDNode;
import jdd.JDDVars;
import leakage.InterLeakComputerBackBisim;
import leakage.InterLeakComputerExp;
import leakage.ProbModelExplicitExplorer;
...
public class Prism extends PrismComponent implements PrismSettingsListener
{
	...
	public void initialise() throws PrismException
	{
		...
		mainLog.print("\nPRIMTL Version: " + getVersion());
		mainLog.print("\nPRISM Version: " + getPrismVersion() + "\n");
	...
	{
		mainLog.println();
		mainLog.println("-------------------------Building Model------------------------------");
		long l; // timer
	...
	/**
	 * Compute intermediate leakage using model checking base on backbisimulation
	 * @param interleakbackbisimVerbose 
	 * @throws PrismException 
	 */
	public void computeInterLeakBackBisim(boolean entropyType, String initDistFileName, boolean interleakbackbisimVerbose) throws PrismException {
		if (currentModelType != ModelType.DTMC) {
			throw new PrismNotSupportedException("Model type not yet supported");
		}
		...
		mainLog.println();
		mainLog.println("------------------------Back Bisimulation----------------------------");
		InterLeakComputerBackBisim leakageComputerBackBisim;
		leakageComputerBackBisim = new InterLeakComputerBackBisim((ProbModel) currentModel, entropyType, initDistFileName, mainLog, interleakbackbisimVerbose);
		printLeakageBackBisim(leakageComputerBackBisim);
		
	}
	/**
	 * Helper function for printing leakage variants using back bisimulation
	 */
	private void printLeakageBackBisim(InterLeakComputerBackBisim leakageComputerBackBisim) {

		double initUncer = leakageComputerBackBisim.initialUncertainty();
		mainLog.println("\nInitial Uncertainty (secret size): " + initUncer + " bits\n");
		double expectedLeakage = leakageComputerBackBisim.expectedLeakage();
	...
	public void printLeakage(InterLeakComputerExp leakageComputer, boolean minmax, boolean bounded, int t) throws PrismException{
		...
		if(!minmax) { // expected leakage
			double expectedLeakage = leakageComputer.expectedLeakage();
			mainLog.println("Expected Leakage: " + expectedLeakage + " bits (" + expectedLeakage*100/initUncer+ " %)\n");
		}
		else { // leakbounds
		
			double maxLeakage = leakageComputer.maxLeakage();
			mainLog.println("Max Leakage: " + maxLeakage + " bits (" + maxLeakage*100/initUncer+ " %)");
			
			double probMaxLeakage = leakageComputer.probMaxLeakage();
			mainLog.println("Prob of Max Leakage: " + probMaxLeakage);
			
			double minLeakage = leakageComputer.minLeakage();
			mainLog.println("Min Leakage: " + minLeakage + " bits (" + minLeakage*100/initUncer+ " %)");
			
			double probMinLeakage = leakageComputer.probMinLeakage();
			mainLog.println("Prob of Min Leakage: " + probMinLeakage);
			
			double probCompleteLeakage = leakageComputer.probCompleteLeakage();
			mainLog.println("Prob of Complete Leakage: " + probCompleteLeakage + "\n");
		}
	...
	{
		return prism.Version.PRIMTLVersionString;
	}
