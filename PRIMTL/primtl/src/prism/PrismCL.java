...
import leakage.InterLeakComputerExp;
...
public class PrismCL implements PrismModelListener
{
    ...
    // PRIMTL flags and variables
	private boolean leak = false;
	private boolean interleakexp = false; //trace-based method
	private boolean interleakbackbisim = false; //back-bisimulation-based method
	private boolean interleakbackbisimVerbose = false; //verbose back-bisimulation-based method
	private boolean interleakexpminmax = false;
	private boolean interleakexpbounded = false;
	private int interleakexpboundedstep = 0;
	private boolean entropyType = InterLeakComputerExp.SHANNON_ENTROPY;
	private boolean initdist = false;
	private String initDistFileName = null;
	...
	public void run(String[] args)
	{
		...
		for (i = 0; i < undefinedMFConstants.getNumModelIterations(); i++) {
			...
			// compute intermediate leakage using back-bisimulation-based method
			if (interleakbackbisim) {
				try {
					prism.computeInterLeakBackBisim(entropyType, initDistFileName, interleakbackbisimVerbose);
            ...
            // compute intermediate leakage using explicit model checking or trace-based method
			if (interleakexp) {
				try {
					prism.computeInterLeakExp(interleakexpminmax, interleakexpbounded, interleakexpboundedstep, 
	...
	private void parseArguments(String[] args) throws PrismException
	{
		...
		for (i = 0; i < args.length; i++) {
			...
			if (args[i].length() > 0 && args[i].charAt(0) == '-') {
                ...
                // final leakage computation
				else if (sw.equals("leak")) {
					steadystate = true;
					leak = true;
				}
				...
				// intermediate leakage computation using back-bisimulation-based method
				else if (sw.equals("backbisim")) {	
					interleakbackbisim = true;			
				}
				// print the characteristics of back-bisimulation quotients
				else if (sw.equals("verbose")) {	
					interleakbackbisimVerbose = true;			
				}
				// min-entropy leakage
				else if (sw.equals("min")) {
					entropyType = InterLeakComputerExp.MIN_ENTROPY;			
				}
				// shannon leakage
				else if (sw.equals("shannon")) {	
					entropyType = InterLeakComputerExp.MIN_ENTROPY;			
				}
				// minimum, maximum leakage, and prob of complete leakage
				else if (sw.equals("leakbounds")) {	
					interleakexpminmax = true;		
				}
				// bounded time leakage
				else if (sw.equals("bounded")) {
					if (i < args.length - 1) {
						interleakexpbounded = true;
						try {
							interleakexpboundedstep = Integer.parseInt(args[++i]);
						} 
						catch (NumberFormatException nfe) {
							errorAndExit("Please enter a valid step number after -t");
					    }
					} else {
						errorAndExit("No bounded step specified");
					}	
					
				}
				// import initial distribution of the secret from file initDistFileName
				else if (sw.equals("initdist")) {	
					if (i < args.length - 1) {
						initdist = true;
						initDistFileName = args[++i];
					} else {
						errorAndExit("No file specified for -initdist");
					}	
				}
				// print PRIMTL help
				else if (sw.equals("help") || sw.equals("h") || sw.equals("?")) {
					if (i < args.length - 1) {
						printHelpSwitch(args[++i]);
					} else {
						printHelp();
	...
	/**
	 * Print a -prismhelp message, i.e. a list of the command-line switches.
	 */
	private void printPrismHelp()
	{
		mainLog.println("Usage: " + Prism.getCommandLineName() + " [options] <model-file> [<properties-file>] [more-options]");
		mainLog.println();
	...
	{
		mainLog.println("Usage: " + Prism.getCommandLineName() + " {-trace|-backbisim} [options] <model-file> [more-options]");
		mainLog.println();
		mainLog.println("Options:");
		mainLog.println("========");
		mainLog.println();
		mainLog.println("-backbisim ..................... Compute the leakage using Back-bisimiulation-based method");
		mainLog.println("-verbose ....................... Verbose the features of quotient model induced by back-bisimulation-based method (only works with the `-backbisim` switch)");
		mainLog.println("-trace ......................... Compute the leakage using trace-based method");
		mainLog.println("-shannon ....................... Compute the expected leakage using Shannon entropy. The default is Shannon entropy");
		mainLog.println("-min ........................... Compute the expected leakage using min-entropy");
		mainLog.println("-leakbounds .................... Compute maximum and minimum leakages, which are upper and lower leakage bounds");
		mainLog.println("-bounded <n> ................... Compute bounded time leakage, which is the amount of expected leakage at a given time (step)");
		mainLog.println("-initdist <file> ............... Specify the initial probability distribution of the secret input. If not specified, the uniform distribution is assumed");
		mainLog.println("-help | -h | -? ................ Display this help message");
		mainLog.println("-prismhelp ..................... Display PRISM help message");
		mainLog.println("-version ....................... Display PRIMTL and PRISM version info");
	}
}